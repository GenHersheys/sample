...
<main class="dashboard-layout">
    <div class="page-header dashboard-header">
        <span class="section-label">Operations Center</span>
    ...
    <div class="dashboard-grid">
        <div class="main-content-column">
            ...
            <div class="card">
                <div class="card-header-row">
                    ...
                    <a href="activities.php" class="card-link">Open drive</a>
                </div>
                <?php if (empty($activity_files)): ?>
                    <p class="empty-state">No files uploaded yet. <a href="activities.php">Manage in Activities</a>.</p>
                <?php else: ?>
                    <ul class="content-list">
        ...
        <div class="sidebar-column">
            ...
            <div class="card">
                <h3>Field Staff</h3>
                <button type="button" class="btn-big btn-checkin btn-checkin-compact" id="checkin-btn">
                    Check-In (Attendance)
                    <small id="geo-status" class="geo-status">Detecting location...</small>
                </button>
                <p class="form-hint" style="margin-top:10px;"><a href="admin/dashboard.php">Open admin dashboard</a></p>
            </div>
            <?php endif; ?>
        </div>
    </div>
...
var IS_STAFF = <?php echo $is_staff ? 'true' : 'false'; ?>;
...
if (IS_STAFF) {
    (function () {
        ...
        if (!btn || !geoStatus) return;
        if (!navigator.geolocation) { geoStatus.textContent = 'Geolocation unavailable'; return; }
        navigator.geolocation.getCurrentPosition(
            function (pos) { geoStatus.textContent = pos.coords.latitude.toFixed(4) + ', ' + pos.coords.longitude.toFixed(4); },
            function () { geoStatus.textContent = 'Location permission denied'; }
        );
        btn.addEventListener('click', function () {
            ...
            navigator.geolocation.getCurrentPosition(function (pos) {
                fetch('api/checkin.php', {
                    ...
                    body: JSON.stringify({ csrf_token: CSRF_TOKEN, latitude: pos.coords.latitude, longitude: pos.coords.longitude })
                }).then(function (r) { return r.json(); }).then(function (data) {
                    flash.className = 'flash flash-' + (data.success ? 'success' : 'error');
                    flash.textContent = data.message;
                }).finally(function () { btn.disabled = false; });
            }, function () {
                flash.className = 'flash flash-error';
                flash.textContent = 'Unable to retrieve location.';
                btn.disabled = false;
            });
        });
    })();
